...
const sources = [
  "./臺北市國道統計車禍數量_精簡.json",
  "./臺北市航空警察統計車禍數量_精簡.json",
  "./臺北市市政府統計車禍傷亡_精簡.json",
];
...
function convertCoords(coords, type) {
  ...
  if (type === "Polygon" || type === "MultiLineString")
    return coords.map((r) => r.map((pt) => proj4("EPSG:3826", "WGS84", pt)));
  if (type === "MultiPolygon")
    return coords.map((p) =>
      p.map((r) => r.map((pt) => proj4("EPSG:3826", "WGS84", pt)))
    );
  return coords;
}
  ...
  chartBar = new Chart(chartBarCanvas.getContext("2d"), {
    ...
    data: {
      ...
      datasets: [
        {
          label: "事故數量",
          data: [stats.A1, stats.A2, stats.other],
          backgroundColor: ["#d73027", "#fdae61", "#2c7bb6"],
        },
      ],
    },
    options: {
...
monthSelect.addEventListener("change", () => {
  if (currentDistrict) updateDistrictStats(currentDistrict);
});
  ...
  map.addLayer(
    {
      ...
      paint: { "fill-color": "#9ab0ff", "fill-opacity": 0.06 },
    },
    "accidents-point"
  );
  map.addLayer(
  ...
  map.addLayer(
    {
      ...
      source: "taipei-districts",
      layout: {
        "text-field": ["coalesce", ["get", "TNAME"], "（未命名）"],
